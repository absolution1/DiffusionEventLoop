...
ReconstructCounterTracks::ReconstructCounterTracks::ReconstructCounterTracks(){
  ...
  fFile = new TFile("fit_counters_tree.root","RECREATE");
  fFile->cd();
  fTree = new TTree("fitpos","fitpos");
  fNCuts = NCuts;
...
ReconstructCounterTracks::~ReconstructCounterTracks(){
  for (std::map<Int_t,std::vector<std::pair<Double_t,Double_t> > >::iterator it = fCounterToLineMap.begin(); it != fCounterToLineMap.end(); it++){
    std::cout<<"For counter " << (*it).first << " there are " << (*it).second.size() << " tracks associated" << std::endl;

  //Now write all of the tracks to text files
    std::ofstream output_file;
    TString output_name = Form("tracks_for_counter_%i.txt",(*it).first);
    output_file.open(output_name.Data());
    for (unsigned int i = 0; i < (*it).second.size(); i++){
      output_file << (*it).second[i].first << " " << (*it).second[i].second << "\n";
    }
    output_file.close();
  }
  fFile->cd();
...
void ReconstructCounterTracks::AccumulateStats(const std::vector<RecoHit> &hits, const std::vector<AuxDet> &aux_dets){
  Bool_t draw = false;
...
std::vector<AuxDet> ReconstructCounterTracks::ReconstructEWTrigger(const std::vector<AuxDet> &aux_dets){
...
Double_t ReconstructCounterTracks::GetT0FromCounters(const std::vector<AuxDet> &aux_dets){
  Double_t T0 = 0;
...
UInt_t ReconstructCounterTracks::NHitsOnWire(UInt_t channel_id, const std::vector<RecoHit> &reco_hits){
  Int_t NHits = 0;
...
Bool_t ReconstructCounterTracks::HitWithinCounterShadow(const RecoHit &hit, const std::vector<AuxDet> &aux_dets){
...
Bool_t ReconstructCounterTracks::PointInPolygon(TVector2 point, const std::vector<TVector2> &polygon_points){
...
Int_t ReconstructCounterTracks::ConvertXToTicks(Double_t x){
  double drift_velocity = 0.109035;

  return (x/drift_velocity)/0.5;
}

Double_t ReconstructCounterTracks::ConvertTicksToX(Int_t t, Bool_t is_short_vol){
...
Bool_t ReconstructCounterTracks::IsInShortDrift(const RecoHit &hit){
  bool flag = false;
...
Double_t ReconstructCounterTracks::Fit2DLine(const std::vector<TVector2> &points, Double_t &m, Double_t &c){
...
void ReconstructCounterTracks::Reset(){
